...
class Entry:
    def __init__(self, xRight, xLeft, yRight, yLeft, yM, xM):
        self.xE = (xRight + xLeft)/2
        self.yE = (yRight + yLeft) / 2
        self.xM = xM
        self.yM = yM
...
def log(x, y):
    print str(x) + ", " + str(y) #send the data to the client
...
while (cap.isOpened()):
    ...
    if ret == True:
        ...
        if xreg != None and yreg != None:
            log(xreg.predict([[(xRight + xLeft)/2]]), yreg.predict([[(yLeft + yRight/2)]]))
